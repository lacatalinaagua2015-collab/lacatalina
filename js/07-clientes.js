...
function ListaClientes({
  ...
  // Datos globales desde el context (fallback a props si vinieran igual) — reduce prop-drilling
  const _ctx = React.useContext(DatosAppContext) || {};
  onPerdida = onPerdida || _ctx.onPerdida;
  todasVentas = todasVentas || _ctx.todasVentas;
  recordatorios = recordatorios || _ctx.recordatorios;
  ...
  const abrirRuta = () => {
    ...
    if (!cp.length) {
      lcAlert("Ningún pendiente tiene Maps cargado.");
      return;
    }
  ...
  const abrirRutaOptima = () => {
    ...
    const conCoords = conMaps.map(c => ({
      c,
      co: extraerCoords(c.maps)
    })).filter(x => x.co);
    if (conCoords.length < 2) {
      lcAlert("Para la ruta óptima necesito al menos 2 clientes pendientes cuyo link de Maps tenga las coordenadas adentro. Si tus links no las tienen, usá la ruta normal (🗺).");
      return;
    }
    ...
    if (afuera > 0) setTimeout(() => lcAlert(`Nota: ${afuera} cliente(s) quedaron afuera de la ruta óptima porque su link de Maps no trae coordenadas.`), 400);
  };
...
function DetalleCliente({
  ...
  // Datos globales desde el context (fallback a props) — reduce prop-drilling
  const _ctx = React.useContext(DatosAppContext) || {};
  onPerdida = onPerdida || _ctx.onPerdida;
  productos = productos || _ctx.productos;
  recordatorios = recordatorios || _ctx.recordatorios;
  // Paginación del historial de movimientos (cargar de a tandas)
  const [limVentas, setLimVentas] = React.useState(40);
  React.useEffect(() => {
    setLimVentas(40);
  }, [cliente && cliente.id]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => {
      if (!razonAjuste.trim()) {
        lcAlert("Ingresá la razón del ajuste");
        return;
      }
    ...
    return todo.slice(0, limVentas).map((item, idx) => {
      // ── NO VISITA ──
      if (item._esNoVisita) {
      ...
      if (esCobro) return /*#__PURE__*/React.createElement("div", {
        ...
        onClick: () => {
          window.lcConfirm("¿Eliminar este cobro?", {
            peligro: true
          }).then(function (ok) {
            if (ok) onEliminarVenta(v.id);
          });
        }
      }, "Eliminar")));
      if (esAjuste) return /*#__PURE__*/React.createElement("div", {
        ...
        onClick: () => {
          window.lcConfirm("¿Eliminar este ajuste?", {
            peligro: true
          }).then(function (ok) {
            if (ok) onEliminarVenta(v.id);
          });
      ...
      if (esCambio) return /*#__PURE__*/React.createElement("div", {
        ...
        onClick: () => {
          window.lcConfirm("¿Eliminar este cambio?", {
            peligro: true
          }).then(function (ok) {
            if (ok) onEliminarVenta(v.id);
          });
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        onClick: () => {
          window.lcConfirm(`¿Eliminar venta de ${fmt(v.neto)}?`, {
            peligro: true
          }).then(function (ok) {
            if (ok) onEliminarVenta(v.id);
          });
  ...
  })(), historial.length + (noVisitas || []).length > limVentas && /*#__PURE__*/React.createElement("button", {
    style: {
      ...s.btn,
      width: "100%",
      padding: "10px",
    ...
    onClick: () => setLimVentas(l => l + 40)
  }, "Cargar más movimientos (", historial.length + (noVisitas || []).length - limVentas, " restantes)"))), /*#__PURE__*/React.createElement("details", {
    style: {
      marginTop: 6
    }
  }, /*#__PURE__*/React.createElement("summary", {
    ...
    onClick: () => {
      ...
      window.lcConfirm(`¿Eliminar a ${cliente.nombre}? Se borrarán también sus ventas.${aviso}`, {
        peligro: true
      }).then(function (ok) {
