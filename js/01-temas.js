...
function _aplicarSombraGlobalLC(tema) {
  ...
  const sombraBtn = oscuro ? "inset 0 1px 0 rgba(255,255,255,0.3), inset 0 -4px 8px rgba(0,0,0,0.5), 0 4px 0 rgba(0,0,0,0.75), 0 7px 12px rgba(0,0,0,0.55)" : "inset 0 1px 0 rgba(255,255,255,0.95), inset 0 -3px 6px rgba(0,0,0,0.14), 0 3px 0 rgba(0,0,0,0.28), 0 5px 10px rgba(0,0,0,0.22)";
  const sombraInput = oscuro ? "inset 0 2px 5px rgba(0,0,0,0.6), inset 0 1px 0 rgba(0,0,0,0.4), 0 1px 0 rgba(255,255,255,0.06)" : "inset 0 2px 4px rgba(0,0,0,0.18), inset 0 1px 0 rgba(0,0,0,0.08), 0 1px 0 rgba(255,255,255,0.8)";
  const sombraBadge = oscuro ? "0 2px 3px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.25)" : "0 2px 3px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.6)";
...
function _capasFibraCarbonoLC(bg, oscuro) {
  ...
  return {
    image: [`linear-gradient(27deg, ${hilo} 5px, transparent 5px)`, `linear-gradient(207deg, ${hilo} 5px, transparent 5px)`, `linear-gradient(27deg, ${hilo} 5px, transparent 5px)`, `linear-gradient(207deg, ${hilo} 5px, transparent 5px)`, `linear-gradient(90deg, ${sombra} 10px, transparent 10px)`, `linear-gradient(${sombra} 25%, transparent 25%, transparent 75%, ${sombra} 75%, ${sombra})`, `linear-gradient(90deg, ${cruce} 10px, transparent 10px)`, `linear-gradient(${cruce} 25%, transparent 25%, transparent 75%, ${cruce} 75%, ${cruce})`].join(", "),
...
function aplicarTemaLC(temaId) {
  ...
  if (!tema) {
    ...
    try {
      localStorage.setItem("lc_tema", JSON.stringify(temaId));
    } catch {}
  }
  const root = document.documentElement;
  Object.entries(tema.vars).forEach(([k, v]) => {
    if (k !== "body-bg") root.style.setProperty(k, v);
  });
...
function getTemaLC() {
  try {
    return JSON.parse(localStorage.getItem("lc_tema") || '"oscuro-azul"');
  } catch {
    return "oscuro-azul";
  }
}
