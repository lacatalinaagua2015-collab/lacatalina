// ════════════════════════════════════════════════════════════════════
// ◆  13-agenda.js — AgendaScreen · NuevoRecordatorioForm · ConfigAparienciaLC
// ════════════════════════════════════════════════════════════════════
...
function AgendaScreen({
  ...
  const tipoCl = {
    visita: "var(--color-text-info)",
    cobro: "var(--color-text-warning)"
  };
  const tipoBg = {
...
function ConfigAparienciaLC() {
  ...
  const aplicar = id => {
    ...
    try {
      ...
      console.warn("Error al aplicar el tema (la elección igual quedó guardada):", e);
    }
    setGuardado(true);
    setTimeout(() => setGuardado(false), 2000);
  };
  const temasFiltrados = Object.entries(TEMAS_LC).filter(([, t]) => t.modo === modoVista && !!t.relieve === (estiloVista === "metalico"));
  ...
  }, [["clasico", "Clásico"], ["metalico", "⚙️ Metálico"]].map(([e, l]) => /*#__PURE__*/React.createElement("button", {
    key: e,
    style: {
      flex: 1,
      padding: "7px 8px",
